...
const PaymentResult = ({ status, title, description, note, primaryCtaLabel, primaryCtaHref, secondaryCtaLabel, secondaryCtaHref,}: PaymentResultProps) => {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-2xl rounded-[28px] border border-[color:var(--color-border)] bg-[color:var(--color-card)] p-6 shadow-[0_40px_90px_-70px_rgba(0,0,0,0.45)] sm:p-10" style={{ background: "radial-gradient(140% 120% at 0% 0%, color-mix(in srgb, var(--color-accent) 12%, transparent), transparent 58%), linear-gradient(180deg, #ffffff, #fbfaf9)", }} >
        <div className="flex flex-col items-center text-center">
          <div className="mb-5 flex h-24 w-24 items-center justify-center rounded-full border border-[color:var(--color-border)] bg-white shadow-[0_20px_40px_-30px_rgba(0,0,0,0.35)]">
            <div className={`flex h-16 w-16 items-center justify-center rounded-full ${ isSuccess ? "bg-[color:var(--color-secondary-bg)]" : "bg-[color:var(--color-bg-soft)]" }`} >
              <div className={`flex h-10 w-10 items-center justify-center rounded-full ${ isSuccess ? "bg-[color:var(--color-accent)]" : "bg-[#1b1b1b]" } text-white`} >
                {isSuccess ? (
          ...
          <div
            className={`mt-7 flex w-full items-start gap-3 rounded-2xl border px-4 py-3 text-left text-sm sm:text-base ${
              isSuccess
                ? "border-[color:var(--color-accent)]/40 bg-[color:var(--color-secondary-bg)] text-[color:var(--color-text)]"
                : "border-[#1b1b1b]/15 bg-white text-[color:var(--color-text)]"
            }`}
          >
            <div className="mt-0.5 flex h-7 w-7 items-center justify-center rounded-full bg-white text-[color:var(--color-accent)] shadow-[0_12px_24px_-18px_rgba(0,0,0,0.35)]">
          ...
          <div className="mt-9 flex flex-wrap justify-center gap-3">
            {primaryHref ? (
              <Link to={primaryHref}>
                <Button type="primary" className="btn primary_btn !h-11 !px-6 !rounded-full !shadow-[0_16px_30px_-20px_rgba(235,74,46,0.7)]" >
                  {primaryLabel}
                </Button>
              </Link>
            ) : (
              <Button
                type="primary"
                className="btn primary_btn !h-11 !px-6 !rounded-full !shadow-[0_16px_30px_-20px_rgba(235,74,46,0.7)]"
              >
                {primaryLabel}
              </Button>
            )}
            ...
            {secondaryHref ? (
              <Link to={secondaryHref}>
                <Button className="btn border-primary_btn !h-11 !px-6 !rounded-full">
                  {secondaryLabel}
                </Button>
              </Link>
            ) : (
