            ...
            <div className="delvoura-cart-scroll flex-1 overflow-y-auto px-5 py-4">
              <div className="mt-2 border-b border-[color:var(--color-border-dark)] pb-5">
                {cartItems.length === 0 ? (
                  ...
                  <div key={`${item.id}-${item.size}`} className={`flex gap-4 pb-5 ${index < cartItems.length - 1 ? "mb-5 border-b border-[color:var(--color-border-dark)]" : ""}`}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button type="button" aria-label="Remove item" className="grid h-8 w-8 place-items-center rounded-full border border-[color:var(--color-border-dark)] text-[color:var(--color-text-on-dark-muted)] transition order-[color:var(--color-accent)] hover:text-[color:var(--color-text-on-dark)]" onClick={() => removeItem(item.id, item.size)}>
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center overflow-hidden rounded-lg border border-[color:var(--color-border-dark)]">
                          <button type="button" className="grid h-8 w-8 place-items-center bg-[color:var(--color-surface-darker)] text-[color:var(--color-text-on-dark)]" onClick={() => updateQty(item.id, item.size, -1)}>
                            -
                          </button>
                ...
                <div className="mt-5">
                  ...
                  <div className="mt-4 space-y-4">
                    {featuredProducts.map((item, idx) => {
                      ...
                      return (
                        <div key={item._id || `${item.name}-${idx}`} className="flex gap-4 rounded-2xl border border-[color:var(--color-border-dark)] bg-[color:var(--color-surface-darker)] p-3">
                          ...
                          <div className="flex-1">
                            ...
                            <button type="button" className="mt-2 text-xs text-[color:var(--color-accent)]" onClick={() => navigate(ROUTES.getProductDetails(item._id || ""))}>
                              Show Details
                            </button>
                          </div>
                          <button type="button" className="delvoura-add-to-cart-btn self-center rounded-full border border-[color:var(--color-accent)] px-3 py-2 text-xs font-semibold text-[color:var(--color-accent)] transition hover:bg-[color:var(--color-secondary-bg)] " onClick={() => { addToCart({ product: item, selectedVariant: size, quantity: 1, image: item.coverimage || item.images?.[0] || "" }); }}>
                            Add To Cart
                          </button>
                        </div>
                      );
