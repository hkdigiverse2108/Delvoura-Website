...
const ProfileCard = ({ variant = "desktop", user: userProp, userData: userDataProp,}: ProfileCardProps) => {
      ...
      <div>
        ...
        {open && (
          <div className="mt-3 flex flex-col gap-2 rounded-xl bg-white p-3" style={{ border: "1px solid #efe6df" }}>
            ...
            <button className="rounded-xl border border-[#efe6df] bg-[#faf7f5] px-4 py-3 text-left text-sm font-semibold text-[#b36a3c]" onClick={() => dispatch(setSignOut())}>
              Logout
            </button>
          </div>
        )}
  ...
  return (
    <Dropdown trigger={["click"]} placement="bottom" overlayClassName="delvoura-profile-dropdown" getPopupContainer={() => document.body} overlayStyle={{ marginTop: 12 }} autoAdjustOverflow={false} dropdownRender={() => (
        <div className="delvoura-profile-menu">
          ...
          <button type="button" className="delvoura-profile-menu-item delvoura-profile-menu-logout" onClick={() => dispatch(setSignOut())} >
            <LogoutOutlined />
            Logout
          </button>
        </div>
